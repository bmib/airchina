...
infosoftglobal.FusionCharts.prototype = {
	...
	setContextPath: function(strContextPath){
		this.setAttribute('contextpath', strContextPath);
	},
	...
	render: function(elementId){
		...
		if((this.detectFlashVersion==1) && (this.installedVer.major < 8)){
			if (this.autoInstallRedirect==1){
				...
				var installationConfirm = window.confirm("您的机器没有合适的FLASH控件，请点击确定打开安装页面。");
				if (installationConfirm){
					window.open(this.getAttribute('contextpath')+'/include/FusionCharts/flashInstall.jsp',null,"left=300,top=300,width=300px,height=200px,status=no,scroll=no");
				}else{
					return false;
				}
			}else{
